...
const csvReader = (csvPath: string): Promise<object[]> => {
  let data = [];
...
const addData = async (graph: Graph<string>, busClass: string) => {
  let firstidxpass = 0;

  await csvReader(`${dataPath}/data/${busClass}_Bus_Route_Detailed.csv`)
    .then((res: object[]) => {
      res.map((item) => {
        if (firstidxpass == 0) {
          firstidxpass += 1;
...
const navigate = async (
  ...
  await addData(graph, 'Express');
  await addData(graph, 'Intercity');
  ...
  result = sortPath(graph, result, depTmn, arrTmn);
  ...
  for (let routeIdx = 0; routeIdx < result.length; routeIdx++) {
    ...
    for (let i = 0; i < (len - 1) / 3; i++) {
      const read: object[] = await csvReader(
